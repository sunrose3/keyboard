...
export class KeyboardDirective implements AfterViewInit {
  ...
  @Output() clickEnter = new EventEmitter<string>();
  ...
  handleFocus(event: MouseEvent): void {
    this.keyboardRef.openKeyboard({ layout: this.layout, entertext: this.entertext, events: { clickEnter: (key: string) => this.clickEnter.emit(key) } });
  }
  ...
  closeKeyboard(event: MouseEvent): void {
    this.keyboardRef.closeKeyboard();
  }
}
